...
public class CreateTimesheetEntryCommand implements SyncCommand {
    ...
    public SyncRecord execute(WebClient webClient, TimesheetService timesheetService, SyncLoggerService syncLoggerService) {
        try {
            ...
            return syncLoggerService.logAndAddSyncRecordWithUpdate(employeeId, new RecordData(loggedHoursByDay.date(),"Updated timesheetLine of employee " + employeeId.name() + " on date " + loggedHoursByDay.date() + " with workOrder " + workOrder.value() + " and hours " + loggedHoursByDay.hours(),  workOrder), loggedHoursByDay.hours());
        } catch (Exception e) {
            if (employeeId.resourceId().isExternal() && timeCode.equals(TimeCode.NO_ASSIGNMENT)) {
                return syncLoggerService.logAndAddSyncRecordWithUpdate(employeeId, new RecordData(loggedHoursByDay.date(),"Could not update timesheetLine of external employee " + employeeId.name() + " BUT OKAY on date " + loggedHoursByDay.date() + " with workOrder " + workOrder.value() + " and hours " + loggedHoursByDay.hours(), workOrder), loggedHoursByDay.hours());
            } else {
